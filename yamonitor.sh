...
#             ***************************              #
#                                                      #
#     @name Yet Another Server Monito                  #
...
function cpu()
{
    ...
    CPU_MODEL_NAME=`cat /proc/cpuinfo | grep -i "^model name" | awk -F": " '{print $2}' | head -1 | sed 's/ \+/ /g'`
    CPU_MODEL=`cat /proc/cpuinfo | grep -i "^model" | awk -F": " '{print $2}' | head -1 | sed 's/ \+/ /g'`
    CPU_FEATURES=`cat /proc/cpuinfo | grep -i "^flags\|^features" | awk -F": " '{print $2}' | head -1 | sed 's/ \+/ /g'`
    CPU_HARDWARE=`cat /proc/cpuinfo | grep -i "^hardware" | awk -F": " '{print $2}' | head -1 | sed 's/ \+/ /g'`
    ...
    echo -e "${!THEME_TEXT}  Features\t ${WHITE}$CPU_FEATURES"
    echo -e "${!THEME_TEXT}  Frequency\t ${WHITE}$CPU_FREQ MHz"
    [[ -z $CPU_CACHE ]] && echo -e "${!THEME_TEXT}  Cache L2\t ${WHITE}-" || echo -e "${!THEME_TEXT}  Cache L2\t ${WHITE}$CPU_CACHE"
...
function showHelp()
{
  echo
  echo "-------"
  echo -e "Name    : $YSM_NAME\nVersion : $YSM_VERSION\nAuthor  : $YSM_AUTHOR\nCreated : $YSM_CREATED"
  echo
  echo -e "$YSM_NAME is originally a PHP project allows you to display system's information of a Unix machine.\nThis is the bash version."
  echo -e "This is a fork of the EzServerMonitor project"
  echo
  echo -e "[USAGE]\n"
  ...
  echo; echo;
  echo -e "More information on : $YSM_URL"
  echo "-------"
  echo
}
